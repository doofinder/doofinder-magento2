...
class Product extends AbstractPlugin
{
    ...
    public function afterSave(ResourceProduct $productResource, $result, AbstractModel $product)
    {
                    ...
                    if ($indexer->isScheduled()) {
                        ...
                        if (!empty($oldstatus) || !empty($newstatus)) {
                            if ($oldstatus != $newstatus) {
                                $this->registration->registerDelete($product->getId(), $store->getCode());

                            }
                        }
                        $this->registration->registerUpdate($product->getId(), $store->getCode());
                        ...
                        $fullAction = $this->indexerHelper->createFullAction($data, $this->indexerHandlerFactory);
                        try {
                        ...
                        try {
                            ...
                            $indexerHandler->saveIndex($dimensions, $fullAction->rebuildStoreIndex($storeId, $productIds));
         ...
         * @return mixed
         */ public
        function afterDelete(ResourceProduct $productResource, $result, AbstractModel $product)
        {
            if ($this->storeConfig->isDoofinderFeedConfigured()) {
                $stores = $this->storeConfig->getAllStores();
                $indexer = $this->indexerRegistry->get(FulltextIndexer::INDEXER_ID);
                    ...
                    if ($this->storeConfig->isUpdateByApiEnable($store->getCode())) {
                        ...
                        if ($indexer->isScheduled()) {
                            ...
                            try {
                                ...
                                $productIds = array_unique(array_merge($entityIds, $this->fulltextResource->getRelationsByChild($entityIds)));
                                //delete
                                $indexerHandler->deleteIndex($dimensions, new ArrayIterator($productIds));
