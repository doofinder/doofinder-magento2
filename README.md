...
> [!NOTE]
> **Using doonGrok:** The FRP tunnel uses `https2http` to terminate SSL externally, so `localAddr` must point to the HTTP port (`9012`), not the HTTPS port (`4012`). Using `4012` causes a "You're speaking plain HTTP to an SSL-enabled server port" error.
> 
> ```toml
> [proxies.plugin]
> type = "https2http"
